...
const Event = () => {
  ...
  // Gallery media items - Replace with your ImageKit URLs
  const mediaItems = [
    {
      id: 1,
      type: "image",
  ...
  return (
    <div className="bg-black pt-16">
      ...
      <div
        className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-12 transition-all duration-1000 ease-out"
        style={{
      ...
      >
        <div className="rounded-xl overflow-hidden">
          <div className="relative h-64 md:h-96 lg:h-128">
            ...
            <button
              onClick={prevSlide}
              className="absolute left-4 top-1/2 transform -translate-y-1/2 w-12 h-12 rounded-full bg-black/50 backdrop-blur-sm flex items-center justify-center text-white hover:bg-black/70 transition-all z-20"
            >
              <ChevronLeft className="w-8 h-8" />
            </button>
            <button
              onClick={nextSlide}
              className="absolute right-4 top-1/2 transform -translate-y-1/2 w-12 h-12 rounded-full bg-black/50 backdrop-blur-sm flex items-center justify-center text-white hover:bg-black/70 transition-all z-20"
            >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-12 overflow-hidden">
        <div
        ...
        >
          {mediaItems.map((item, index) => (
            <div
              key={`row1-${item.id}`}
              className="group relative min-w-64 md:min-w-80 h-48 md:h-64 rounded-lg overflow-hidden flex-shrink-0 transition-all duration-500"
              style={{
      ...
      {/* Animated Showcase Row 3 */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12 overflow-hidden">
        <div
          className="relative flex space-x-4 transition-all duration-1000 ease-out"
          style={{
            opacity: animateItems ? 1 : 0,
            transform: animateItems ? "translateX(0)" : "translateX(30px)",
          }}
        >
          {[...mediaItems]
            .slice(3, 8)
            .concat([...mediaItems].slice(0, 3))
            .map((item, index) => (
              <div
                key={`row3-${item.id}-${index}`}
                className="group relative min-w-64 md:min-w-80 h-48 md:h-64 rounded-lg overflow-hidden flex-shrink-0 transition-all duration-500"
                style={{
                  transform: `translateX(-${secondaryIndex * 25}%)`,
                  transition: "transform 1s ease-out",
                }}
              >
                {item.type === "image" ? (
